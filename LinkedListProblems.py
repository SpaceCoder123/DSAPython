...
class LinkedListProblems:
    ...
    def insertInMiddle( self, head, x):
        ...
        temp = head
        middle = self.getMiddleElement(temp)
        var = middle.next
    ...
    def removeNodes(self, head):
        curr = head
        reversedHead = self.reverseList(curr)
        curr = reversedHead
        highest = float("-inf")
        prev = None
        while curr:
    ...
    def pairSum(self, head):
        ...
        middle.next = None
        reversedList = self.reverseList(nextElement)
        highestElement = 0
        while(temp!=None):
    ...
    def isSubPath(self, head, root):
        ...
        return self.checkLinkedList(head,root) or self.isSubPath(head, root.left) or self.isSubPath(head, root.right)

    def checkLinkedList(self, head, root):
        if head is None:
            return True
        if root is None:
            return False
        ...
        left = self.checkLinkedList(head.next, root.left)
        right = self.checkLinkedList(head.next, root.right)
